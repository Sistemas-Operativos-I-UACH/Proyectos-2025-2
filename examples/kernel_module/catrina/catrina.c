...
#define DEVICE_NAME "catrina"
//#define CLASS_NAME  "catrinaclass"
...
static int catrina_open(struct inode *, struct file *);
static int catrina_close(struct inode *, struct file *);
static ssize_t catrina_read(struct file *, char __user *, size_t, loff_t *);
static ssize_t catrina_write(struct file *, const char *, size_t, loff_t *);

// Define file operations structure
static struct file_operations fops = {
    .open = catrina_open,
    .release = catrina_close,
    .read = catrina_read,
    .write = catrina_write
};
 ...
 */
static ssize_t catrina_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
...
static ssize_t catrina_write(struct file *filep, const char *buffer, size_t len, loff_t *offset) {
    int bytes_written = 0;
...
// Open function
static int catrina_open(struct inode *inode, struct file *file) {
    device_open_count++;
...
// Close function
static int catrina_close(struct inode *inode, struct file *file) {
 ...
 */
static int __init catrina_load(void)
{
    // printk is the kernel equivalent of printf
    // KERN_INFO is the log level
    printk(KERN_INFO "Ya llegó la catrina a jalarte las patas\n");
 ...
 */
static void __exit catrina_unload(void)
{
    printk(KERN_INFO "No le saque!!\n");
}

// Register the entry and exit functions
module_init(catrina_load);
module_exit(catrina_unload);
