...
// Module metadata
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Los Chinchillescos");
MODULE_DESCRIPTION("Shows messages and creates its own device file");

#define DEVICE_NAME "Stampede"
#define CLASS_NAME  "StampedeClass"
...
static char kernel_buffer[BUFFER_SIZE] = "Despues de todo esta tierra esta hecha de amor y paz.\n";
static char stream_buffer[BUFFER_SIZE] = "Esta hecha de amor y paz.\n";

static char write_buffer[BUFFER_SIZE];

static int Stampede_open(struct inode *, struct file *);
static int Stampede_close(struct inode *, struct file *);
static ssize_t Stampede_read(struct file *, char __user *, size_t, loff_t *);
static ssize_t Stampede_write(struct file *, const char *, size_t, loff_t *);

// Define file operations structure
static struct file_operations fops = {
    .open = Stampede_open,
    .release = Stampede_close,
    .read = Stampede_read,
    .write = Stampede_write
};

/**
 * @brief Called when data is read from the device file.
 */
static ssize_t Stampede_read(struct file *filp, char __user *buf, size_t len, loff_t *off) {
    ...
    if(exit == 0) {
        ...
        if ( stream ) {
            ...
            bytes_to_read = strlen(stream_buffer);
            errors += copy_to_user(buf + total_bytes, stream_buffer, bytes_to_read);
            total_bytes += bytes_to_read;
            //i++;
...
static ssize_t Stampede_write(struct file *filep, const char *buffer, size_t len, loff_t *offset) {
    int bytes_written = 0;
...
// Open function
static int Stampede_open(struct inode *inode, struct file *file) {
    device_open_count++;
...
// Close function
static int Stampede_close(struct inode *inode, struct file *file) {
    printk(KERN_INFO "Cerrando dispositivo.\nNo es facil escapar del pasado que no puedes enterrar.\n");
    return 0;
}
 ...
 */
static int __init Stampede_load(void)
{
    // printk is the kernel equivalent of printf
    // KERN_INFO is the log level
    printk(KERN_INFO "El boleto al futuro siempre esta en blanco.\n");

    /* 1. Allocate a device number (Major, Minor) */
    if (alloc_chrdev_region(&device_numbers, 0, 1, DEVICE_NAME) < 0) {
        printk(KERN_ALERT "Stampede: Device Number could not be allocated\n");
        return -1;
    }
    printk(KERN_INFO "Stampede: Device Numbers Major: %d, Minor: %d was registered\n",
           MAJOR(device_numbers), MINOR(device_numbers));

    /* 2. Create Device Class */
    if ((my_class = class_create(CLASS_NAME)) == NULL) {
        printk(KERN_ALERT "Stampede: Class could not be created\n");
        goto ClassError;
    }
    ...
    if (device_create(my_class, NULL, device_numbers, NULL, DEVICE_NAME) == NULL) {
        printk(KERN_ALERT "Stampede: Device could not be created\n");
        goto FileError;
    }
    ...
    if (cdev_add(&my_cdev, device_numbers, 1) == -1) {
        printk(KERN_ALERT "Stampede: Registering of device to kernel failed\n");
        goto AddError;
    }
 ...
 */
static void __exit Stampede_unload(void) {

    cdev_del(&my_cdev);
    device_destroy(my_class, device_numbers);
    class_destroy(my_class);
    unregister_chrdev_region(device_numbers, 1);
    printk(KERN_INFO "Stampede: No hay descanso para mi en mi busqueda de paz.\n");

}

// Register the entry and exit functions
module_init(Stampede_load);
module_exit(Stampede_unload);
