...
int main(int argc, char *argv[]) {
    ...
    if(strlen(argv[1]) > (unsigned long)&num-(unsigned long)buf){
        printf("El contenido a almacenar en el buffer podria desbordarlo.\n\n");
    }
    ...
    strncpy(buf, argv[1], sizeof(buf)-1); // -> No se podra, ya que se esta limitando el contenido a copiar en el buffer. 
    buf[sizeof(buf)-1] = '\0';            // -> Agregamos manualmente el fin de cadena.
