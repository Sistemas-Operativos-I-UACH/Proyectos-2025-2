...
int main(int argc, char *argv[]) {
    ...
    *txt = 3.141592;
    ...
    if(strlen(argv[1])+1 <= malloc_usable_size(buf)){
        printf("El puntero no se desbordara.\n");
    } else {
        printf("El puntero se desbordara. Accederemos fuera de sus limites.\n");
    }

    if(strlen(argv[1])+1 <= (unsigned long)txt-(unsigned long)buf){
